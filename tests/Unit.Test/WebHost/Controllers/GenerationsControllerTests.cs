...
{
    public class GenerationsControllerTests
    {
        private readonly Mock<ICarGenerationService> _serviceMock;
        private readonly GenerationsController _controller;

        public GenerationsControllerTests()
        {
            _serviceMock = new Mock<ICarGenerationService>();
            _controller = new GenerationsController(_serviceMock.Object);
        }
        ...
        public async Task GetById_ShouldReturnOk_WhenExists()
        {
            // Arrange
            var id = Guid.NewGuid();
            var expectedResponse = GenerateCarGenerationResponse(id);
        ...
        public async Task GetById_ShouldReturnNotFound_WhenMissing()
        {
            ...
            _serviceMock
                .Setup(x => x.GetByIdAsync(id, It.IsAny<CancellationToken>()))
                .ReturnsAsync((CarGenerationResponse?)null);
        ...
        {
            // Arrange
            var request = GenerateCarGenerationResponses(3);
        ...
        {
            // Arrange
            var request = GenerateCreateCarGenerationRequest();
            var createdId = Guid.NewGuid();
            var expectedResponse = new CarGenerationResponse { Id = createdId, Name = request.Name };
        ...
        public async Task Update_ShouldReturnOk()
        {
            // Arrange
            var id = Guid.NewGuid();
            var request = GenerateUpdateCarGenerationRequest();
            var updatedResponse = new CarGenerationResponse { Id = id, Name = request.Name };
        ...
        private List<CarGenerationResponse> GenerateCarGenerationResponses(int count = 3)
        {
            return new Faker<CarGenerationResponse>()
                .RuleFor(b => b.Id, f => f.Random.Guid())
                .RuleFor(b => b.Name, f => f.Company.CompanyName())
                .Generate(count);
        }

        private CarGenerationResponse GenerateCarGenerationResponse(Guid id)
        {
            return new Faker<CarGenerationResponse>()
                .RuleFor(b => b.Id, id)
                .RuleFor(b => b.Name, f => f.Company.CompanyName())
        ...
        private CreateCarGenerationRequest GenerateCreateCarGenerationRequest()
        {
        ...
        private UpdateCarGenerationRequest GenerateUpdateCarGenerationRequest()
        {
