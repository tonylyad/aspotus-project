...
const navItems = [
  { to: '/', label: 'Панель управления', end: true, roles: ['Admin', 'Operator', 'ContentModerator'] },
...
export default function Layout() {
  ...
  return <div className="layout">
    <aside className="sidebar">
      <div className="sidebar-header"><h2>Aspotus Admin</h2></div>
      <nav className="sidebar-nav">{visibleNavItems.map((item) => <NavLink key={item.to} to={item.to} end={item.end}
        className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>{item.label}</NavLink>)}</nav>
    </aside>
    <div className="main-area">
      <header className="topbar"><span className="topbar-title">Панель управления</span><div className="topbar-right">
        <span className="topbar-user">{localStorage.getItem('fullName')}</span><button className="logout-btn" onClick={handleLogout}>Выйти</button>
      </div></header>
